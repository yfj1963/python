...
def savainclude(filename,runLog) :
    ...
    try:
        ...
        if len(lstring) ==0:
            return
        preListLine = 'fffffffffffaaaaaaaaaaa'
        for lstringItem in range(0,lstring.__len__()):
            #print(len(lstring))
            #if lstringItem == 0:
...
def writeOldFile(filename,lstrpre,lstrcurrent):
    print('enterwriteoldfile')
    with open(filename,'r') as oldfile:
        oldfilelist = oldfile.readlines()
        oldfile.close()
    tarline = 0
    for i in range(0, oldfilelist.__len__()):
        if oldfilelist[i] == lstrpre or lstrpre == 'fffffffffffaaaaaaaaaaa':
            print(oldfilelist[i],'and',lstrpre)
            tarline = int(i) + 1
            oldfilelist.insert(tarline,lstrcurrent)
            if lstrpre == 'fffffffffffaaaaaaaaaaa':
                lstrpre = 'fffff'
...
if __name__=='__main__':
    filepath="D:\\script\\pythonfile\\test.h"
    runLog = "D:\\script\\pythonfile\\runLog.txt"
